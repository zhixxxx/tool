...
class HttpUpload {
    ...
    public $tmpName; //在服务器上的临时文件(路径+文件:C:\WINDOWS\TEMP\php3A.tmp)
    public $oriName; //原来的文件名
    public $oriSize; //文件大小
    public $oriError; //$_FILES['userfile']['error']错误
    public $N; //文件名称
    public $T; //文件的后缀名 如(jpg)
    public $F; //上传成功后的文件全称(=$N.".".$T)
    public $errorNum; //错误代码
    public $errorMsg; //错误信息(数组)
	...
	*例子：
	*/
    public function getFile($_tmpname, $_oriname, $_orisize, $_orierror) {
	...
	*/
    public function checkFile($_extension = "", $_maxsize = "", $_width = "", $_height = "") {
        if (! $this->isUploaded ( $this->tmpName, $this->oriError ))
            return false;
        if (! $this->checkExtension ( $this->oriName, $_extension ))
            return false;
        if (! $this->checkSize ( $this->oriSize, $_maxsize ))
            return false;
        if (! $this->checkWH ( $this->tmpName, $_width, $_height ))
            return false;
        if (! is_numeric ( $this->errorNum )) {
            $this->errorNum = 5;
        } //防止单文件变多文件上传
        return true;
    }
	...
	*/
     ...
     *例子：
     */
    public function checkExtension($_file, $_extension) {
	...
	*/
    /*大小判断 单位:字节*/
    public function checkSize($_size, $_maxsize) {
        if ($_size > $_maxsize) {
            $this->errorNum = 7;
            return false;
        }
	...
	*例子：
	*/
    public function checkWH($_file, $_width = "", $_height = "") {
        $fileinfo = getimagesize ( $_file );
        if ($_width != "") {
            if ($fileinfo [0] > $_width) {
                $this->errorNum = 8;
                return false;
            }
        }
        if ($_height != "") {
            if ($fileinfo [1] > $_height) {
                $this->errorNum = 9;
                return false;
            }
        }
	...
	*/
    public function upLoad($_newname = "", $_newpath = "") {

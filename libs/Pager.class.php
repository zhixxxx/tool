 ...
 * 模式四种分页模式：
    require_once('./Pager');
    $objPage=new Pager(array('total'=>1000,'perpage'=>20));
    echo 'mode:1<br>'.$objPage->show();
    echo '<hr>mode:2<br>'.$objPage->show(2);
    echo '<hr>mode:3<br>'.$objPage->show(3);
    echo '<hr>mode:4<br>'.$objPage->show(4);
    开启AJAX：
...
class Pager {
     ...
     * @return string
     */
    function pre_bar($style = '') {
        if ($this->nowindex > 1) {
            $temp = (($this->nowindex - 10) > 1) ? ($this->nowindex - 10) : 1;
            return $this->_get_link ( $this->_get_url ( $temp ), $this->pre_bar, $style );
        }
     ...
     * @return string
     */
    function next_bar($style = '') {
        if ($this->nowindex < $this->totalpage) {
            $temp = (($this->totalpage - $this->nowindex) < 10) ? $this->totalpage : ($this->nowindex + 10);
            return $this->_get_link ( $this->_get_url ( $temp ), $this->next_bar, $style );
        }
     ...
     */
    function select() {
        $return = '<select name="PB_Page_Select" onChange="window.location.replace(\'' . $this->url . '\'+this.options[this.selectedIndex].value);">';

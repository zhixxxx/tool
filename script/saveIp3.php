...
foreach($res as $val){
	...
	$temp1 = explode('.',$ip1);
	$temp2 = explode('.',$ip2);
	$var1 = $temp1[0].'.'.$temp1[1].'.'.$temp1[2].'.0';
	$var2 = $temp2[0].'.'.$temp2[1].'.'.$temp2[2].'.0';
	$var1int = bindec(decbin(ip2long($var1)));
	$var2int = bindec(decbin(ip2long($var2)));
	if($var1int < $var2int){
		$tempip2 = $temp1[0].'.'.$temp1[1].'.'.$temp1[2].'.255';
	...
	if($var1int != $var2int){
		...
		while($ttempa <= $b){
			...
			if($ttempa + 256 > $b){
				...
				$arrIn['ip2'] = long2ip($ttempa + 255);
				$arrIn['ipe'] = $ttempa + 255;
			}
       		$rs = $db->insert('ip_in',$arrIn);
        	if (! $rs){

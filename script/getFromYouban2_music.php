...
define("DATA_NEWID_FILE",'youban_17.txt');//youban_6.txt,youban_5.txt|youban_4.txt|youban_4.txt|youban_2.txt|youban_1.txt
...
$tempArr = $newarr;
$imgs=$mp3s=$swfs=$imgf=$mp3f=$swff=0;
foreach($tempArr as &$val){
	//是否写入图片处理
	$info = explode('|',$val['img']);
	if($info[0] != '' && $info[1] == ''){
		$imgname = time().'_'.basename($info[0]);
		echo "get:".$info[0];
		if(copy($info[0],DATA_NEWID.'/pic/'.$imgname)){
			$val['img'] = $val['img'].'|'.$imgname;
			$imgs++;
			echo "---ok\n";
		}else{
			$imgf++;
	...
	//是否写入mp3处理
	$info = explode('|',$val['mp3']);
	if($info[0] != '' && $info[1] == ''){
		$imgname = time().'_'.$val['title']."_".basename($info[0]);
		echo "get:".$info[0];
		if(copy($info[0],DATA_NEWID.'/mp3/'.$imgname)){
			$val['mp3'] = $val['mp3'].'|'.$imgname;
			$mp3s++;
			echo "---ok\n";
		}else{
			$mp3f++;
	...
	//是否写入swf处理
	$info = explode('|',$val['swf']);
	if($info[0] != '' && $info[1] == ''){
		$imgname = time().'_'.$val['title']."_".basename($info[0]);
		echo "get:".$info[0];
		if(copy($info[0],DATA_NEWID.'/flash/'.$imgname)){
			$val['swf'] = $val['swf'].'|'.$imgname;
			$swfs++;
			echo "---ok\n";
		}else{
			$swff++;
...
$count = count($tempArr);
echo $imgs."_".$mp3s."_".$swfs."_".$imgf."_".$mp3f."_".$swff."_".$count;
exit;
function saveArr($tempArr){
	foreach($tempArr as $val){
		$arr = array();
		foreach($val as $k=>$v){
			$arr[] = $k.'=>'.$v;
